...
/
(3 * 2^(1/3) * (PI * r12 - 2 * PI * r1 * r2 + PI * r22))
-
(2^(1/3) * (2 * pi2 * h2 * r14 - 
            4 * pi2 * h2 * r13 * r2 +
            2 * pi2 * h2 * r12 * r22))
...
// volume, spans, space(angle), prerotate
module dohash(h, r1, angle, wall, poly=0)
{
    r2 = r2(h, r1, angle);

    for (hash = hash_sets[HashSet]) {
        volume = hash[0];
        spans = hash[1];
        space = hash[2];
        prerot = hash[3];

        h2 = vtoh(oz_to_ml(volume)*1000, h, r1, r2);
        if (h2 <= h) {
            shape = (poly == 0) ?
